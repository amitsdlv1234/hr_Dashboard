const OrganizationRepository = require('./model/organization.modal.repository');
const AddService = require('./services/organization.add.service');
...
exports.addOrganization=async(req,res)=>{
    try {
        ...
        const present=await organizationRepository.checkOrganizationExists(userId);
       console.log(present)
        if(present) res.status(202).json({message:"user Data is already register"});
        else{
           const addService=new AddService();
           const newUser=await addService.addOrganization(user);
            ...
            try {
               ...
               console.log(present)
                if(!present) res.status(202).json({message:"user Data is not present"});
                   ...
                   const addService=new AddService();
                   const newUser=await addService.updateOrganization(user);
