...
const expenseRepository = require('./model/expense.modal.repository');
const AddService = require('./services/expense.add.service');
...
exports.addexpense=async(req,res)=>{
    ...
    //     const present=await ExpenseRepository.checkexpenseExists(userId);
    //    console.log(present)
    //     if(present) res.status(202).json({message:"user Data is already register"});
    //     else{
           const addService=new AddService();
           const newUser=await addService.addexpense({...user,employeeName:name,requestedBy:name});
            ...
            try {
               ...
               console.log(present)
                if(!present) res.status(202).json({message:"user Data is not present"});
                   ...
                   const addService=new AddService();
                   const newUser=await addService.updateexpense(user);
