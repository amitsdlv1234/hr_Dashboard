const UserEducationDetailsRepository = require('./model/userEducation.modal.repository');
const AddService = require('./services/educationData.add.service');
...
exports.addEducationDetails=async(req,res)=>{
    try {
        ...
        const userEducationDetailsRepository= new UserEducationDetailsRepository();
        const present=await userEducationDetailsRepository.checkUserEducationDetailsExists(userId);
       console.log(present)
        if(present) res.status(202).json({message:"user Data is already register"});
        else{
           const addService=new AddService();
           const newUser=await addService.addEducationDetails(user);
            ...
            try {
               ...
               console.log(present)
                if(!present) res.status(202).json({message:"user Data is not present"});
                   ...
                   const addService=new AddService();
                   const newUser=await addService.updateEducationDetails(user);
